...
import { getArticleList, getTagData } from "@/lib/service";
...
export default function TagPage() {
  ...
  const fetchData = async () => {
    let res = null;
    res = await getArticleList(`tag_id=${id}&page=${page}`);
    setData(res);
    res = await getTagData(id);
    setInfo(res);
    document.title = "标签: " + res.name;
  };
      ...
      {data.articles.length>0 && (
        <div className="bg-white shadow-sm mt-3 p-2 flex justify-between">
          <Button
            ...
            onClick={() =>
              router.push(
                `/tag?id=${id}&page=${parseInt(page) - 1}`
              )
            }
          >
            上一页
