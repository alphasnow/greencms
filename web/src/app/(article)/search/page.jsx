...
export default function SearchPage() {
  ...
  const fetchData = async () => {
    let res = null;
    res = await getArticleList(`keywords=${keywords}&page=${page}`);
    setData(res);
    document.title = "搜索: " + keywords;
  };
  ...
  return (
    <main className="container px-0 mx-auto mt-3 min-h-screen">
      ...
      <div className="mt-3">
        <ArticleList list={data.articles} grid="block" keywords={keywords} />
      </div>
      ...
      {data.articles.length>0 && (
        <div className="bg-white shadow-sm mt-3 p-2 flex justify-between">
          <Button
            ...
            onClick={() =>
              router.push(
                `/search?keywords=${keywords}&page=${parseInt(page) - 1}`
              )
            }
          >
            上一页
